...
Chained_Node* lookup(string variable_name, string function_scope, Chained_Node* symbol_table[]) {
    ...
    if (!function_scope) {
        ...
        Chained_Node *fnct = lookup_hash(variable_name, NULL, functions_definitions); // ← cherche variable_name comme FONCTION
        if (fnct) {  
            cn = lookup_hash(variable_name, function_scope, fnct->symbol_table);
            if(cn) return cn;
            return lookup_hash(function_scope, "global", global_variable); }
        return NULL; // ← retourne NULL si variable_name n'est pas une fonction
    }
}
